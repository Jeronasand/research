...
预览打开后只显示登录页。这里没有后端服务。浏览器通过生成短期有效的私有数据源 manifest 签名 `GET` URL 来校验访问权限，成功后再为选中的调研文档生成签名 URL。

仓库不会提交真实密钥。数据桶不使用公开 ACL；浏览器端 OSS 读取仍需要 CORS 允许预览页面 origin 发起 `GET`。
