# TradingBot 金融/Crypto/股票数据源产品待调研
...
本版本按用户提供的 `financial_crypto_equity_data_sources` 清单入库，清单标注的 `last_verified_at` 为 2026-05-11。本轮未逐个重新验证所有官方链接的实时状态。
...
| R2 专项情报 | Nansen, Arkham, Kaito, Santiment, Intrinio, Nasdaq Data Link, TuShare, AKShare, JQData | 钱包画像、实体归因、社交叙事、基本面、另类数据、中国市场 | 先验证目标场景是否真的需要，再评估价格和商用边界 |
...
| P1 | TuShare Pro | A股 / 港股待补 | A股、基金、指数、期货、宏观 | A股数据、中国市场量化、本地化金融数据 | 商用授权、分钟级行情、财务质量、积分和 rate limit |
| P1 | AKShare | A股 / 开源数据接口 | 股票、期货、期权、基金、外汇、债券、指数、Crypto | 原型验证、A股数据探索、非正式数据补充、学术研究 | 数据源授权、生产稳定性、接口维护成本、商业适配 |
